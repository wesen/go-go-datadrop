...
## Step 8: Two things that were nobody's fault and everybody's problem
...
**Commit (code):** `cbd71ae` — "Stop Biome and Go fighting over the shared projection fixture"

### The formatter war

`TestWriteLiveProjectionFixture` in `pkg/tabular` had been failing since before
DATADROP-8 and DATADROP-9 started. Both agents checked, correctly, that
`git log f53be15..HEAD -- pkg/tabular` was empty and concluded it was
pre-existing and none of their business. I concluded the same, twice, and
"fixed" it once by regenerating the fixture.

It is a conflict between two formatters over one shared file:

```console
$ GOWORK=off go test ./pkg/tabular -run TestWriteLiveProjectionFixture -update
  # writes scalar arrays expanded, one element per line (Go 1.26)
$ bun run --cwd=ui lint:fix          # biome check --write
  # collapses them — test/** is in biome's includes
$ GOWORK=off go test ./pkg/tabular
  FAIL  the shared projection fixture is stale
```

Whoever ran a formatter last decided whether the other suite passed. My earlier
regeneration held exactly until the next `lint:fix`, which is why it came back
looking untouched.

The file is generated by the Go test, so Go owns its format; Biome now excludes
it. Verified by running `-update` then `lint:fix` and confirming the fixture
survives, with both suites green simultaneously.

**The lesson is about the diagnosis, not the fix.** "Untouched by any commit in
this range" felt like proof it was not our problem. It only proved no *commit*
caused it. A file mutated by two tools outside version control has no commit to
point at, and the check that would have found it — run one tool, then the other,
then look — takes ten seconds and neither of us ran it.

### The reMarkable upload did not happen

Attempted five times across the session. Every attempt fails the same way:

```text
ERROR: failed to build documents tree, last error: failed to mirror was not ok: request failed with status 400
```

Not a rate limit, not authentication. `RMAPI_TRACE=1 rmapi ls` shows
`GET /sync/v4/root` returning 200 — the credentials work — and then several
`GET /sync/v3/files/<hash>` requests returning 400 while others return 200. The
client cannot assemble the document tree, so `mkdir` and `put` never get as far
as transferring anything.

`rmapi version` reports `dev`, a locally built binary. The most likely
explanation is a client/cloud API drift on the sync v3 blob endpoint, which is
outside this repository and not fixable from here.
