...
- [ ] Phase 1 — De-singleton the store: makeStore seeds its own document, save/load take a key, usePersistence, and a guard test that the module exports no instance <!-- t:6nz9 -->
- [ ] Phase 2 — Split WorkbenchShell from the application: move the signed-out gate, the ?first=1 read and the persistence effect up; drop height:100vh; add WorkbenchInstance and the per-instance app allow-list <!-- t:c22i -->
- [ ] Phase 3 — The fixture base query: serve tables from a fixture map on the store's extra argument, honour the row budget, and round-trip every endpoint back to a SourceRef <!-- t:ihfj -->
